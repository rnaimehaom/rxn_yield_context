...
if __name__ == '__main__':
    ...
    """
    Get the Reaxys ID versus Reaction Type dictionary:
    """
    reaxys_dir = "../../Data_From_Reaxys_Original"
    type_file = os.path.join(args_r.test_dir, 'reaction_types.txt')
    f = open(type_file, 'r')
    dir_names = f.readlines()
    f.close()
    for i in range(len(dir_names)):
        dir_names[i] = dir_names[i].strip('\n')
    ...
    #              'Pauson Khand Reaction', 'Reductive Amination', 'Suzuki Coupling'] # Change Here
    reaxys_id_dict = dict() # This variable is the dictionary we want to search reaction type
    
    def basic_preprocess(data_path):
        keep_index = ['Reaction ID']
    ...
    multiple_rxn_NDCG = []
    temp_hit_diffs = []
    hit_ratio_dict = dict() # For evaluation of how many ectries in the data can be predicted
    
    SEPARATE_ACCURACY = dict()
    for dir_name in dir_names:
        SEPARATE_ACCURACY.update({dir_name: []})
    
    for i, rxn in enumerate(data[:]):
        reaxys_id = rxn[0]
        reaction_type = reaxys_id_dict.get(reaxys_id) # maybe None
        ...
        acc_list.append(id_)
        if reaction_type:
            SEPARATE_ACCURACY[reaction_type].append(id_)
    ...
    print('\n')
    print('-'*50)
    print('Separately evaluating on all the different reaction types:')
    for z, dir_name in enumerate(dir_names):
        print('Reaction Type {}, {}:'.format(z+1, dir_name))
        print('Reaction entries: {}'.format(len(SEPARATE_ACCURACY[dir_name])))
        evaluate_overall(SEPARATE_ACCURACY[dir_name])
        print('-'*50)