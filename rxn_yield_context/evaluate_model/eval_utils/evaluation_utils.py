...
from rxn_yield_context.train_multilabel.data_utils import Yield2Relevance, create_rxn_Morgan2FP_concatenate
...
class MultiTask_Evaluator:
    ...
    """
    def __init__(self, solvent_classes, reagent_classes, cutoff_solv = 0.2, cutoff_reag = 0.25):
    ...
    def enumerate_combinations(self, rxn_fp):
        ...
        p2 = list(combinations([i for i in p2 if i not in self.EXC_REAG_INDEX], 2)) + list(combinations(p2, 1))
    ...
    def make_input_rxn_condition(self, rxn_fp):
        ...
        enumerated_solvent = list(zip(*enumerated_features))[0]
        enumerated_reagent = list(zip(*enumerated_features))[1]
    ...
    def make_input_rxn_conditionBYnames(self, rxn_fp):
        ...
        candidates_solv = list(combinations([x for x in candidates_solv if x not in self.exc_solv_names], 2)) + list(combinations(candidates_solv, 1))
        candidates_reag = list(combinations([x for x in candidates_reag if x not in self.exc_reag_names], 2)) + list(combinations(candidates_reag, 1))
...
class Ranking_Evaluator():
    ...
    def rank_top_contexts(self, rxn_fp, input_solvents, input_reagents):
        ...
        top_contexts = [sort_string(contexts[j]) + [temperatures[j]] for j in top_index] # TODO : check
...
def compare_all_answers(answers, context_combinations):
    ...
    for answer in answers:
        ...
        for hit, context in enumerate(context_combinations):
            solvent, reagent, temp_pred = context
            solvent = set(solvent.split('; '))
            reagent = set(reagent.split('; '))
            if (answer_s == solvent) & (answer_r == reagent):
...
def table_for_contexts(rxn, top_contexts): # TODO: change!!!!
    ...
    my_table.field_names = ['Yield / Rank', 'Reagent(s)', 'Solvent(s)', 'Temperature']
    
    gold_context = rxn[3]
    gold_context = sort_string(gold_context)
    predictions = [ [ 'Rank{}'.format(i+1), top_contexts[i][1], top_contexts[i][0], '{:.1f}'.format(top_contexts[i][2]) ] for i in range(len(top_contexts))]
